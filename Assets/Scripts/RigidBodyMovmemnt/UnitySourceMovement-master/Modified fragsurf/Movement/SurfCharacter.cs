    ...
    public class SurfCharacter : NetworkBehaviour, ISurfControllable {
        ...
        private void UpdateMoveData () {
            ...
            if (toggleSprintEnabled)
            {
                ...
                else if (Input.GetButtonDown("Sprint") && sprinting)
                {
                    sprinting = false;
                }
                if (Input.GetAxisRaw("Vertical") == 0)
                {
                    if (Input.GetAxisRaw("Horizontal") != 0 && sprinting) { sprinting = true; }
                    else { sprinting = false; }
                }
                if (Input.GetAxisRaw("Vertical") < 0) { sprinting = false; }

                _moveData.sprinting = sprinting;

            }
            else { _moveData.sprinting = (Input.GetAxisRaw("Vertical") < 0) ? false : Input.GetButton("Sprint"); }
